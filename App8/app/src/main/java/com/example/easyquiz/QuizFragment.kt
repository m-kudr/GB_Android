...
import com.example.easyquiz.databinding.FragmentQuizBinding
...
class QuizFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.linearLayout.animate().apply {
            duration = 3000
            alpha(1f)
        }.start()
    ...
    private fun getAnswersByUser(): Int {
        ...
        if (binding.question1.checkedRadioButtonId == binding.answer14.id) correctAnswersCount++
        if (binding.question2.checkedRadioButtonId == binding.answer22.id) correctAnswersCount++
        if (binding.question3.checkedRadioButtonId == binding.answer35.id) correctAnswersCount++
